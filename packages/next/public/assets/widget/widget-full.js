        ...
        function (module, __webpack_exports__, __webpack_require__) {
            ...
            var promise_ZalgoPromise = (function () {
                ...
                _proto.reject = function (error) {
                    ...
                    if (!error) {
                        var _err =
                            error && 'function' == typeof error.toString ? error.toString() : {}.toString.call(error);
            ...
            function awaitFrameLoad(frame) {
                if (
                    (awaitFrameLoadPromises = awaitFrameLoadPromises || new weakmap_CrossDomainSafeWeakMap()).has(frame)
                ) {
            ...
            function getBridgeName(domain) {
                return (
                    ...
                    (domain = domain || getDomainFromUrl(domain)).replace(/[^a-zA-Z0-9]+/g, '_')
                );
            }
            function isBridge() {
                                        ...
                                        if (!matchDomain(domain, origin))
                                            throw new Error(
                                                "Method '" +
                                                    ...
                                                    "' domain " +
                                                    JSON.stringify(
                                                        util_isRegex(meth.domain) ? meth.domain.source : meth.domain
                                                    ) +
            ...
            var send_send = function send(win, name, data, options) {
                ...
                return promise_ZalgoPromise
                    ...
                    .then(function (targetDomain) {
                        domain = targetDomain;
                        var logName =
                            'postrobot_method' === name && data && 'string' == typeof data.name
                                ? data.name + '()'
                                : name;
            ...
            function cleanUpWindow(win) {
                for (
                    ...
                    _requestPromises$get2[_i2]
                        .reject(
                            new Error('Window ' + (isWindowClosed(win) ? 'closed' : 'cleaned up') + ' before response')
                        )
                        .catch(src_util_noop);
            }
            var src_bridge;
                ...
                return inlineMemoize(
                    ...
                    function () {
                        ...
                        try {
                            ...
                            throw new Error(
                                'Can not decode window name payload: ' + encodedPayload + ': ' + stringifyError(err)
                            );
                        }
                    },
                    [windowName]
                            ...
                            .try(function () {
                                ...
                                var buildWindowNamePromise = getProxyWindowPromise.then(function (proxyWin) {
                                    return (function (_ref5) {
                                        ...
                                        return (
                                            '__zoid__' + name + '__' + base64encode(JSON.stringify(childPayload)) + '__'
                                        );
                                    })({
                                        proxyWin: proxyWin,
                                        childDomain: childDomain,
            ...
            function component_component(opts) {
                var options = (function (options) {
                    var tag = options.tag,
                        ...
                        containerTemplate =
                            void 0 === _options$containerTem ? defaultContainerTemplate : _options$containerTem,
                        _options$prerenderTem = options.prerenderTemplate,
                        prerenderTemplate =
                            void 0 === _options$prerenderTem ? defaultPrerenderTemplate : _options$prerenderTem,
    ...
    if (_ppSetupWidgets.length === 1) {
        window.PurplePassZoidPopup = zoid.create({
            ...
            url: ({ props }) => {
                let url =
                    PP_BASEURL +
                    'order/checkout?widget=true&theme=' +
                    parameters.theme +
                    '&lang=' +
                    parameters.language;
        ...
        window.PurplePassZoidAsePopup = zoid.create({
            ...
            url:
                PP_BASEURL +
                'events/_ase_window?widget=true&theme=' +
                parameters.theme +
                '&lang=' +
                parameters.language,
            dimensions: { width: '0%', height: '0%' },
