    ...
    function (module, __webpack_exports__, __webpack_require__) {
            ...
            for (var i = 0; i < handlers.length; i++) {
              ...
              if (_result2 instanceof ZalgoPromise && (_result2.resolved || _result2.rejected)) {
                ...
                utils_isPromise(_result2)
                  ? _result2 instanceof ZalgoPromise && (_result2.resolved || _result2.rejected)
                    ? _result2.resolved
      ...
      function onResize(el, handler, _temp) {
        ...
        var check = function () {
          ...
          ((width && newWidth !== currentWidth) || (height && newHeight !== currentHeight)) &&
            handler({
              width: newWidth,
              height: newHeight,
            });
        ...
        var observer;
        var timeout;
        if (void 0 !== win.ResizeObserver) (observer = new win.ResizeObserver(check)).observe(el);
            ...
            if (
              ...
              windowStore('remoteBridgeAwaiters').getOrSet(win, function () {
                return promise_ZalgoPromise.try(function () {
                  ...
                  if (frame)
                    return isSameDomain(frame) && global_getGlobal(assertSameDomain(frame))
                      ? frame
          ...
          setName: function (name) {
            return winPromise.then(function (win) {
              ...
              var frame = (function (win) {
                ...
                for (
                  var _i21 = 0, _document$querySelect2 = document.querySelectorAll('iframe');
          ...
          globalStore('builtinListeners').getOrSet('functionCalls', function () {
            return on(
              ...
              function (_ref2) {
                ...
                if (!meth)
                  throw new Error(
                    "Could not find method '" + name + "' with id: " + data.id + ' in ' + getDomain(window)
                  );
                    ...
                    if (!matchDomain(domain, origin))
                      throw new Error(
                        "Method '" +
                          data.name +
                          "' domain " +
                          JSON.stringify(util_isRegex(meth.domain) ? meth.domain.source : meth.domain) +
      ...
      function deserializeMessage(source, origin, message, _ref2) {
        ...
        return (function (str, deserializers) {
          ...
          message,
          (((_deserialize = {}).cross_domain_zalgo_promise = function (serializedPromise) {
      ...
      SEND_MESSAGE_STRATEGIES.postrobot_post_message = function (win, serializedMessage, domain) {
        (Array.isArray(domain) ? domain : 'string' == typeof domain ? [domain] : ['*'])
          .map(function (dom) {
              ...
              if (domainListeners) {
                if (domain && 'string' == typeof domain) {
                  ...
                  if (domainListeners.__domain_regex__)
                    for (
                      var _i6 = 0, _domainListeners$__DO2 = domainListeners.__domain_regex__;
      ...
      var RECEIVE_MESSAGE_TYPES =
        (((_RECEIVE_MESSAGE_TYPE = {}).postrobot_message_request = function (source, origin, message, _ref) {
          var on = _ref.on,
            send = _ref.send;
          ...
          var logName =
            'postrobot_method' === message.name && message.data && 'string' == typeof message.data.name
              ? message.data.name + '()'
              : message.name;
        ...
        var requestListener = (function addRequestListener(_ref4, listener) {
          ...
          if (existingListener)
            throw win && domain
              ...
              : domain
              ? new Error('Request listener already exists for ' + name + ' on domain ' + domain.toString())
                ...
                : promise_ZalgoPromise
                    ...
                    .then(function (normalizedDomain) {
                      if (!matchDomain(targetDomain, targetDomain))
                        throw new Error(
                          'Domain ' + stringify(targetDomain) + ' does not match ' + stringify(targetDomain)
                        );
              ...
              var interval = safeInterval(function () {
                if (isWindowClosed(win))
                  return promise.reject(
                    new Error('Window closed for ' + name + ' before ' + (responseListener.ack ? 'response' : 'ack'))
                  );
      ...
      function cleanUpWindow(win) {
        for (
          var _i2 = 0, _requestPromises$get2 = windowStore('requestPromises').get(win, []);
          _i2 < _requestPromises$get2.length;
          _i2++
        )
          _requestPromises$get2[_i2]
            .reject(new Error('Window ' + (isWindowClosed(win) ? 'closed' : 'cleaned up') + ' before response'))
            .catch(src_util_noop);
      }
      var src_bridge;
      src_bridge = {
        ...
        var getPropsForChild = function (domain) {
          ...
          for (var _i2 = 0, _Object$keys2 = Object.keys(props); _i2 < _Object$keys2.length; _i2++) {
            ...
            (prop && !1 === prop.sendToChild) ||
              (prop && prop.sameDomain && !matchDomain(domain, getDomain(window))) ||
                    ...
                    var win = (function (url, options) {
                      ...
                      width &&
                        (window.outerWidth
                          ...
                          : window.screen.width && (left = Math.round((window.screen.width - width) / 2)));
                      height &&
                        (window.outerHeight
                          ? (top = Math.round((window.outerHeight - height) / 2) + window.screenY)
                          : window.screen.height && (top = Math.round((window.screen.height - height) / 2)));
                      width &&
                        height &&
                        (options = _extends(
                          {
          ...
          !(function (propsDef, props, inputProps, helpers, isUpdate) {
            ...
            for (var _i4 = 0; _i4 < propNames.length; _i4++) {
              ...
              if (propDef) {
                ...
                if (
                  isDefined(value) &&
                  ('array' === propDef.type ? !Array.isArray(value) : typeof value !== propDef.type)
                )
        ...
        var updateProps = function (newProps) {
          ...
          return initPromise.then(function () {
            ...
            if (childComponent && proxyWin)
              return childComponent.updateProps(getPropsForChild(getDomainMatcher())).catch(function (err) {
        ...
        return {
          ...
          render: function (target, container, context) {
            return promise_ZalgoPromise
                        ...
                        if (prop)
                          return promise_ZalgoPromise
                            ...
                            .then(function (value) {
                              if (null != value)
                                return promise_ZalgoPromise
                                  .all([props_getQueryParam(prop, key, value), getQueryValue(prop, 0, value)])
                                  .then(function (_ref) {
                                    ...
                                    if ('boolean' == typeof queryValue) result = queryValue.toString();
                                      ...
                                      if (prop.serialization === PROP_SERIALIZATION.JSON)
                                        ...
                                        prop.serialization === PROP_SERIALIZATION.DOTIFY ||
                                        !prop.serialization
                                      ) {
                                        ...
                                        for (
                                          var _i12 = 0, _Object$keys8 = Object.keys(result);
              ...
              .catch(function (err) {
                return promise_ZalgoPromise.all([onError(err), destroy(err)]).then(
                  function () {
                    throw err;
                  },
                  function () {
          ...
          if (isChild()) {
            ...
            var child = (function (options) {
              ...
              if ('9_0_41' !== childPayload.version)
                throw new Error(
                  'Parent window has zoid version ' + childPayload.version + ', child window has version 9_0_41'
                );
              ...
              var parentComponentWindow = (function (ref) {
                ...
                if ('global' === type && ref.uid && 'string' == typeof ref.uid) {
                  ...
                  for (
                    var _i2 = 0, _getAllFramesInWindow2 = getAllFramesInWindow(ancestor);
              ...
              var setProps = function (newProps, origin, isUpdate) {
                ...
                var normalizedProps = (function (parentComponentWindow, propsDef, props, origin, helpers, isUpdate) {
                  ...
                  if (!isUpdate)
                    for (var _i4 = 0, _Object$keys4 = Object.keys(propsDef); _i4 < _Object$keys4.length; _i4++) {
                      var _key = _Object$keys4[_i4];
                      props.hasOwnProperty(_key) ||
                        (result[_key] = normalizeChildProp(propsDef, 0, _key, void 0, helpers));
                    }
                  return result;
                })(
...
/* widget.js */
let PP_BASEURL =
  document.location.href.indexOf('127.0.0.1') >= 0 || document.location.href.indexOf('localhost') >= 0
    ? 'http://localhost/'
    : 'https://www1.purplepass.com/';
if (document.location.href.indexOf('stage.') >= 0) {
  PP_BASEURL = 'http://purplepass.stage.reaktivate.com/';
}
const MODAL_TEMPLATE = `
  ...
  <style>
    ...
    #pp-modal-ase-window.visible,
    #pp-modal-window.visible {
        display: block;
    }
    ...
    ${window.screen.width < 800 ? '#pp-modal-ase-window {top:0; left:0; width: 100%; height: 100% }' : ''}
    #pp-modal-ase-backdrop.visible,
    #pp-modal-backdrop.visible {
      display: block;
    }
  </style>
`;
...
function PurplePassWidget(parameters) {
  ...
  let url = `${PP_BASEURL}widget?xprops=${JSON.stringify(parameters)}&lang=${parameters.language}&theme=${
    parameters.theme
  }`;
  ...
  window[widgetName] = zoid.create({
    ...
    dimensions: {
      ...
      height: parameters.height ? parameters.height + 'px' : '500px',
    },
  ...
  if (_ppSetupWidgets.length === 1) {
    window.PurplePassZoidPopup = zoid.create({
      ...
      url: ({ props }) => {
        let url = PP_BASEURL + 'order/checkout?widget=true&theme=' + parameters.theme + '&lang=' + parameters.language;

        if (props.target === 'event' || props.target === 'calendar-day') {
          url = PP_BASEURL + `widget?theme=` + parameters.theme + '&lang=' + parameters.language;
        }
    ...
    window.PurplePassZoidAsePopup = zoid.create({
      ...
      url: PP_BASEURL + 'events/_ase_window?widget=true&theme=' + parameters.theme + '&lang=' + parameters.language,
      dimensions: { width: '0%', height: '0%' },
    });

    parameters.ppClosePopup = () => {
      document.querySelector('#pp-modal-backdrop').classList.remove('visible');
      document.querySelector('#pp-modal-window').classList.remove('visible');
      document.querySelector('#pp-modal-window').innerHTML = '';
    };

    ppCloseAsePopup = () => {
      document.querySelector('#pp-modal-ase-backdrop').classList.remove('visible');
      document.querySelector('#pp-modal-ase-window').classList.remove('visible');
      document.querySelector('#pp-modal-ase-window').innerHTML = '';
    };

    document.body.innerHTML += MODAL_TEMPLATE;

    document.querySelector('#pp-modal-backdrop').addEventListener('click', () => {
      parameters.ppClosePopup();
    });

    document.querySelector('#pp-modal-ase-backdrop').addEventListener('click', () => {
      // ppCloseAsePopup();
    });
  }
  ...
  parameters.onGoToCheckout = () => {
    document.querySelector('#pp-modal-backdrop').classList.add('visible');
    document.querySelector('#pp-modal-window').classList.add('visible');
    window.PurplePassZoidPopup({ test: true }).render('#pp-modal-window');
    setTimeout(() => {
      const el = document.getElementById('pp-modal-window').getElementsByTagName('div')[0];
      el.style.width = '100%';
      el.style.height = '80vh';
    }, 1000);
  };

  parameters.onGoToEventPanel = (id, slug) => {
    document.querySelector('#pp-modal-backdrop').classList.add('visible');
    document.querySelector('#pp-modal-window').classList.add('visible');
    const parametersNew = {
      target: 'event',
      ...
      uxMode: 'builtin',
      bgNotTransparent: parameters.bgNotTransparent,
      onGoToAse: onGoToAse,
      ppClosePopup: parameters.ppClosePopup,
      isPanel: true,
    };
    window.PurplePassZoidPopup(parametersNew).render('#pp-modal-window');
    setTimeout(() => {
      const el = document.getElementById('pp-modal-window').getElementsByTagName('div')[0];
      el.style.width = '100%';
      el.style.height = '80vh';
  ...
  parameters.onGoToCalendarDayPanel = ({ openedCalendarDay, fieldSearch, startDate }) => {
    document.querySelector('#pp-modal-backdrop').classList.add('visible');
    document.querySelector('#pp-modal-window').classList.add('visible');
    const parametersNew = {
      target: 'calendar-day',
  ...
  const onGoToAse = (data) => {
    document.querySelector('#pp-modal-ase-backdrop').classList.add('visible');
    document.querySelector('#pp-modal-ase-window').classList.add('visible');
    data.onClosePopup = ppCloseAsePopup;
    data.isASE = true;
    window.PurplePassZoidAsePopup(data).render('#pp-modal-ase-window');
    setTimeout(() => {
      const el = document.getElementById('pp-modal-ase-window').getElementsByTagName('div')[0];
      el.style.width = '100%';
      el.style.height = '100%';
      el.style.maxHeight = '100%';
    }, 1000);
  };
  parameters.onGoToAse = onGoToAse;
  window[widgetName](parameters).render(parameters.container);
}

function pp_replaceNamespace(original, newName) {
  var replacement = document.createElement(newName);
...
function pp_startWidget() {
  const _elements = document.getElementsByTagName('pp:tickets');
  const _elementsLists = document.getElementsByTagName('pp:eventlist');
  const _elementsListsCal = document.getElementsByTagName('pp:calendar');
  if (!_elements.length && !_elementsLists.length && !_elementsListsCal.length) return;
  const elements = [..._elements, ..._elementsLists, ..._elementsListsCal];
  for (var i = 0; i < elements.length; i++) {
    let element = elements[i];
    // if (!element.attributes) element = element.item(0);
    const newName = element.tagName.replace(':', '').toLowerCase();
    //console.log('E', element, element.item(0).attributes);
    pp_replaceNamespace(element, newName);
    let widgetType = 'event-detail';
    if (newName === 'ppeventlist') widgetType = 'events-list';
    if (newName === 'ppcalendar') widgetType = 'events-calendar';

    const options = {
      eventId: element.getAttribute('event'),
      organizerId: element.getAttribute('event-all'),
      trackingId: element.getAttribute('data-sid'),
      showHidden: element.getAttribute('data-hidden-events'),
      topText: element.getAttribute('data-top-box'),
      widgetType: widgetType,
      container: newName,
      theme: element.getAttribute('data-etheme'),
      // width: element.getAttribute('width') || 600,
      // height: element.getAttribute('height'),
    };
    PurplePassWidget(options);
  }
}
